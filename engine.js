...
async function loadDB() {
  ...
  // Derive an absolute URL to exams.json from engine.js's own <script src>.
  // engine.js is always at  <root>/js/engine.js
  // exams.json is always at <root>/data/exams.json
  // Stripping "/js/engine.js" gives us the repo root regardless of which
  // page (root or pages/) triggered the load.
  let jsonURL = 'data/exams.json'; // safe relative fallback for root pages
  const scripts = document.querySelectorAll('script[src]');
  for (const s of scripts) {
    if (s.src && s.src.includes('/js/engine.js')) {
      const root = s.src.replace(/\/js\/engine\.js(\?.*)?$/, '');
      jsonURL = root + '/data/exams.json';
      break;
    }
  }

  const res = await fetch(jsonURL);
  if (!res.ok) throw new Error('Could not load exam database from: ' + jsonURL);
  DB = await res.json();
  return DB;
}
